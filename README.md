...
- 观察nFeature_RNA可视化结果，在使用比较低的分辨率的时候某一群细胞是否出现一个奇怪的状态，例如下图的第一个细胞群（它明显出现了低表达的细胞群，以及高表达的细胞群）；当我们把这一群细胞提高分辨率之后，观察UMAP里，是否被细分为两个亚群，如果是的话，它就不是双细胞，并同时观察nFeature里面是否分割出低表达的细胞群和高表达的细胞群（理想情况下。。）。    
<img src="https://github.com/y741269430/scRNAseq-flow/blob/main/img/nFeatureRNA.png" width="400" />    
